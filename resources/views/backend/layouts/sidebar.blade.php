      ...
      <ul class="navbar-nav" id="navbar-nav">
         ...
         @foreach($menus as $menu)
            ...
            @if($menu->childrenRecursive->isEmpty())
               <li class="nav-item">
                  <a class="nav-link" href="{{ $menu->resolved_url ?? '#' }}">
                     <span class="nav-icon">
                     ...
                     <ul class="nav sub-navbar-nav">
                        @foreach($menu->childrenRecursive as $child)
                           ...
                           <li class="sub-nav-item">
                              <a class="sub-nav-link" href="{{ $child->resolved_url ?? '#' }}">
                              ...
                              @if($child->childrenRecursive->isNotEmpty())
                                 <ul class="nav sub-navbar-nav ms-3">
                                    @foreach($child->childrenRecursive as $subChild)
                                       @php $subColor = $colors[array_rand($colors)]; @endphp
                                       <li class="sub-nav-item">
                                          <a class="sub-nav-link" href="{{ $subChild->resolved_url ?? '#' }}">
                                             <iconify-icon icon="{{ $subChild->icon ?? 'mdi:circle-small' }}" style="color:{{ $subColor }};"></iconify-icon>
                                             {{ $subChild->name }}
                                          </a>
                                       </li>
                                    @endforeach
