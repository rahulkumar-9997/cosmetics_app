...
class CheckMenuAccess
{
    public function handle(Request $request, Closure $next)
    {
        ...
        if ($assignedMenus->isEmpty()) {
            abort(403, 'Unauthorized access. No menu assigned.');
        }
        $menu = Menu::where('url', $currentRoute)->first();
        if ($menu)
        {
            $hasAccess = $menu->roles()->whereIn('roles.id', $user->roles->pluck('id')->toArray())->exists() || $menu->permissions()->whereIn('permissions.id', $user->getAllPermissions()->pluck('id')->toArray())->exists();
            if (!$hasAccess)
            {
